...
namespace TcpGameClient
{
    internal class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                RequestServerLogon(client);
                Thread.Sleep(10000); // temp until refactor of this client.
                RequestNewGame(client);
        ...
        private static void RequestNewGame(GameClient client)
        {
            var newGameRequest = new ClientRequest
            {
                RequestName = "lobby.newgame",
            };
            SendRequest<ClientRequest>(client, newGameRequest);
        }

        private static void RequestServerLogon(GameClient client)
        {
